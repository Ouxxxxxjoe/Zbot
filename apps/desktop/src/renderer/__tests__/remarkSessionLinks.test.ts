...
const SESSION_URL = 'zbot://session/03e0c22d-19db-4ac5-814f-1ea04040b471';
...
describe('remarkSessionLinks', () => {
  ...
  it('leaves legacy project links with raw delimiters as plain text (review P2)', () => {
    ...
    expect(runOnText('zbot://project/%2Ftmp%2Ffoo(copy)')).toEqual([
      { type: 'text', value: 'zbot://project/%2Ftmp%2Ffoo(copy)' },
    ]);
    expect(runOnText("zbot://project/%2FJohn's%20Repo")).toEqual([
      { type: 'text', value: "zbot://project/%2FJohn's%20Repo" },
    ]);
  });

  it('ignores unknown xdt-maker URL shapes and malformed ids', () => {
    expect(runOnText('zbot://other/foo')).toEqual([
      { type: 'text', value: 'zbot://other/foo' },
    ]);
    // 非法 % 序列解析不出 sessionId → 不切
    expect(runOnText('zbot://session/%ZZ')).toEqual([
      { type: 'text', value: 'zbot://session/%ZZ' },
    ]);
  });

  // 双 scheme 收敛:主 scheme zbot:// 链接与历史 zbot://(上方全部用例)
  // 同一口径切 link 节点。
  it('converts primary-scheme zbot:// session and project URLs', () => {
    const cindySession = 'zbot://session/03e0c22d-19db-4ac5-814f-1ea04040b471';
