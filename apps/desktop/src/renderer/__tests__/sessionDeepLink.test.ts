...
describe('buildProjectDeepLink(严格编码,review P2)', () => {
  it('escapes !\'()* so generated links never contain bare matcher delimiters', () => {
    ...
    expect(href.startsWith('zbot://project/')).toBe(true);
    expect(/[!'()]/.test(href.slice('zbot://project/'.length))).toBe(false);
...
describe('parseSessionDeepLinkHref', () => {
  it('parses a bare session href', () => {
    expect(parseSessionDeepLinkHref('zbot://session/abc-123')).toEqual({
      sessionId: 'abc-123',
  ...
  it('ignores empty or malformed message anchor but keeps session id', () => {
    expect(parseSessionDeepLinkHref('zbot://session/abc?message=')).toEqual({
      sessionId: 'abc',
      messageClientId: null,
      deviceId: null,
    });
    expect(parseSessionDeepLinkHref('zbot://session/abc?message=%ZZ')).toEqual({
      sessionId: 'abc',
      messageClientId: null,
      deviceId: null,
    });
    expect(parseSessionDeepLinkHref('zbot://session/abc?foo=1&message=m1')).toEqual({
      sessionId: 'abc',
      messageClientId: 'm1',
      deviceId: null,
    });
  ...
  it('strips fragments and trailing slashes', () => {
    expect(parseSessionDeepLinkHref('zbot://session/abc/#frag')).toEqual({
      sessionId: 'abc',
      messageClientId: null,
      deviceId: null,
    });
  ...
  it('rejects non-session hrefs and empty ids', () => {
    expect(parseSessionDeepLinkHref('zbot://project/dir')).toBeNull();
    expect(parseSessionDeepLinkHref('zbot://session/')).toBeNull();
    expect(parseSessionDeepLinkHref('https://example.com')).toBeNull();
    expect(parseSessionDeepLinkHref('zbot://session/%ZZ')).toBeNull();
  });
});
...
describe('session deep link device parameter', () => {
  it('freezes deviceId into the link and roundtrips through the parser', () => {
    ...
    expect(href).toBe('zbot://session/abc-123?device=dev-mac~1');
  ...
  it('combines message anchor and device parameter', () => {
    ...
    expect(href).toBe('zbot://session/abc?message=client%2F9&device=dev-1');
    expect(parseSessionDeepLinkHref(href)).toEqual({
      sessionId: 'abc',
      messageClientId: 'client/9',
      deviceId: 'dev-1',
  ...
  it('omits the parameter for local sessions (null/undefined deviceId)', () => {
    expect(buildSessionDeepLink('abc', { deviceId: null })).toBe('zbot://session/abc');
    expect(buildSessionMessageDeepLink('abc', 'm1', {})).toBe('zbot://session/abc?message=m1');
  });

  it('ignores empty or malformed device values but keeps the rest of the link', () => {
    expect(parseSessionDeepLinkHref('zbot://session/abc?device=')).toEqual({
      sessionId: 'abc',
      messageClientId: null,
      deviceId: null,
    });
    expect(parseSessionDeepLinkHref('zbot://session/abc?device=%ZZ&message=m1')).toEqual({
      sessionId: 'abc',
      messageClientId: 'm1',
      deviceId: null,
    });
...
// 双 scheme 收敛:生成一律 zbot://,解析 cindy + 历史 xdt-maker 都认
// (上面的 zbot:// 用例即历史 scheme 回归)。
describe('dual scheme (cindy primary + legacy xdt-maker)', () => {
  it('builders emit the primary zbot:// scheme', () => {
  ...
  it('parsers accept primary-scheme zbot:// hrefs', () => {
    expect(parseSessionDeepLinkHref('zbot://session/abc-123?message=m1')).toEqual({
      sessionId: 'abc-123',
      messageClientId: 'm1',
      deviceId: null,
    });
    expect(parseProjectDeepLinkHref('zbot://project/%2Ftmp%2Fx')).toEqual({
      workingDir: '/tmp/x',
    });
    expect(parseSessionDeepLinkHref('zbot://session/')).toBeNull();
  });
