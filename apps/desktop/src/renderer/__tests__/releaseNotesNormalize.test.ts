...
describe('release-notes normalization', () => {
  ...
  it('同一版本重复选择同一语言时选择对应内容,raw payload 只请求一次', async () => {
    ...
    for (let i = 0; i < 2; i += 1) {
      const notes = await mod.fetchReleaseNotes('0.1.23', 'zh-CN');
      expect(notes?.intro).toBe('intro-zh-CN');
  ...
  it('同一版本并发请求同一语言时也只跨 IPC 请求一次 raw payload', async () => {
    ...
    const first = mod.fetchReleaseNotes('0.1.28', 'zh-CN');
    const second = mod.fetchReleaseNotes('0.1.28', 'zh-CN');
    expect(fetchReleaseNotes).toHaveBeenCalledTimes(1);
    ...
    resolveRaw({
      ...
      date: '2026-08-06',
      contentByLocale: {
        'zh-CN': { topics: [{ id: 'same', title: '简体标题', text: '简体正文。' }] },
      },
    });

    await expect(first).resolves.toMatchObject({ topics: [{ title: '简体标题' }] });
    await expect(second).resolves.toMatchObject({ topics: [{ title: '简体标题' }] });
    expect(fetchReleaseNotes).toHaveBeenCalledTimes(1);
  });

  it('zh-CN localized 内容缺失或畸形时回退到顶层旧格式', async () => {
    stubFetch({
      version: '0.1.24',
      date: '2026-08-06',
    ...
    expect((await mod.fetchReleaseNotes('0.1.24', 'zh-CN'))?.topics[0]?.text).toBe('legacy-root');
  });

  it('localized block 为 null 或非对象时按缺失处理并继续 fallback', async () => {
    let fetchReleaseNotes = stubFetch({
      version: '0.1.29',
      date: '2026-08-06',
      topics: [{ title: '顶层中文', text: 'legacy-root' }],
      contentByLocale: {
    ...
    expect((await mod.fetchReleaseNotes('0.1.29', 'zh-CN'))?.topics[0]?.text).toBe('legacy-root');
    expect(fetchReleaseNotes).toHaveBeenCalledTimes(1);

    vi.resetModules();
    fetchReleaseNotes = stubFetch({
      version: '0.1.31',
      date: '2026-08-06',
      topics: [{ title: '顶层中文', text: 'legacy-root' }],
      contentByLocale: {
    ...
    const nextMod = await import('@/release-notes');
    expect((await nextMod.fetchReleaseNotes('0.1.31', 'zh-CN'))?.topics[0]?.text).toBe(
      'legacy-root',
    );
  });

  it('zh-CN localized block 无有效内容时继续回退到 legacy 顶层内容', async () => {
    stubFetch({
      version: '0.1.30',
      date: '2026-08-06',
      topics: [{ title: '顶层说明', text: '有效的顶层内容。' }],
      contentByLocale: {
        'zh-CN': { topics: [] },
      },
    });
    ...
    await expect(mod.fetchReleaseNotes('0.1.30', 'zh-CN')).resolves.toMatchObject({
      topics: [{ title: '顶层说明', text: '有效的顶层内容。' }],
    });
  });

  it('zh-CN localized 缺失时直接回退顶层中文', async () => {
    stubFetch({
      version: '0.1.26',
      date: '2026-08-06',
      topics: [{ title: '顶层中文', text: 'legacy-root' }],
    ...
    expect((await mod.fetchReleaseNotes('0.1.26', 'zh-CN'))?.topics[0]?.text).toBe('legacy-root');
  });
  ...
  it('localized legacy sections 也能被选择和归一化', async () => {
    ...
    const mod = await import('@/release-notes');
    const notes = await mod.fetchReleaseNotes('0.1.27', 'zh-CN');
    expect(notes?.sections).toEqual([
      { title: 'Bug Fixes', items: [{ text: '已修复', by: 'A' }] },
    ]);
  });
