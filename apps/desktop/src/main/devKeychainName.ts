 ...
 * (service = `<app.name> Safe Storage`),而 `app.name` 默认取 productName(zagent),
 ...
 *    识别时,沙箱可能已有按 zagentDev 主密钥加密的存量密文,静默回退默认身份会用
 *    错钥匙覆盖它们——dev-only 场景响亮失败并给出处置指引,优于任何猜测。
 *  - 无标记(确证 ENOENT)且 profile 已有真实数据(排除标记自身产物)→ 复查标记
 *    后判旧沙箱,永久保持默认条目名(存量密文绑定 `zagent Safe Storage`)。
 ...
 *    目录)进入观察模式:绝不认领 zagentDev,但目录已带标记时依标记运行——裸覆写
 *    指向已按 zagentDev 认领的 `-dev2` 沙箱是受支持形态,若因缺隔离旗标就跳过标记、
 *    以默认身份打开,同一 profile 会被两种身份轮流打开互毁密文(review 反馈 P1
 *    第十四轮)。标记不可读/不可识别同样 abort。
 *  - 空 profile 的身份在**两种模式下都原子落定**:认领模式认领 zagentDev,观察模式
 *    认领默认身份(标记词表 = 两个身份名),输家依胜者标记。观察模式若对空 profile
 *    直接保持默认名而不落标记,与并发隔离启动的 zagentDev 认领无协调点,两个进程会
 ...
 * 词表 = 'zagentDev' | 默认身份名 zagent,其它内容一律视为身份不确定)。
 */
...
function interpretMarker(
  ...
  if (read.kind === 'present') {
    ...
    // "zagentDev\n" 的前 6 字节恰是词表另一合法身份 "zagent":只 trim 的话,并发读端
...
export function resolveDevKeychainDecision(input: {
  ...
  // (--isolated + 纪元派生目录)认领 zagentDev,观察模式(裸覆写等非认领形态)
  // 认领默认身份。观察模式对空 profile 若直接 keep-default 而不落标记,与并发
  // 隔离启动的 zagentDev 认领之间没有任何协调点,两个进程会对同一 profile 以两种
