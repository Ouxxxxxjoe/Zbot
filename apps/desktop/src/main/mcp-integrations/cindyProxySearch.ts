...
function classifyHttpFailure(
  ...
  if (status === 401 || status === 403) {
    return {
      ...
      message: 'Zbot AI 搜索鉴权失败，请重新登录或稍后再试',
    };
  }
  ...
  if (looksLikeQuota) {
    return {
      ...
      message: 'Zbot AI 搜索额度不足，请稍后再试或在插件设置中改用自己的搜索渠道',
    };
  }
  if (status === 404) {
    return {
      errorCode: 'NOT_CONFIGURED',
      message: 'Zbot AI 搜索服务尚未配置，请稍后再试',
    };
  }
  if (
    ...
    return {
      errorCode: 'NOT_CONFIGURED',
      message: 'Zbot AI 搜索模型尚未配置，请稍后再试',
    };
  }
  if (status === 429) {
    return {
      errorCode: 'RATE_LIMITED',
      message: 'Zbot AI 搜索请求过于频繁，请稍后再试',
    };
  }
  if (status >= 500) {
    return {
      errorCode: 'UPSTREAM_UNAVAILABLE',
      message: 'Zbot AI 搜索服务暂时不可用，请稍后再试',
    };
  }
  if (status === 400 || status === 422) {
    return {
      errorCode: 'INVALID_PARAMS',
      message: 'Zbot AI 搜索请求参数未被服务接受',
    };
  }
  return {
    errorCode: 'INTERNAL',
...
function invalidSearchResponse(): ParsedSearchResponse {
  return {
    ...
    message: 'Zbot AI 搜索返回了无法识别的结果，请稍后再试',
  };
}
...
function searchToolFailure(errorCode: unknown): ParsedSearchResponse {
  ...
  if (errorCode === 'max_uses_exceeded') {
    return {
      ok: false,
      errorCode: 'RATE_LIMITED',
      message: 'Zbot AI 搜索已达到本次调用上限，请稍后再试',
    };
  }
    ...
    async search({ query, limit }): Promise<CindyProxySearchOutcome> {
      ...
      if (!baseUrl || !apiKey) {
        return {
          ok: false,
          errorCode: 'NOT_CONFIGURED',
          message: 'Zbot AI 搜索尚未就绪，请重新登录或在插件设置中改用自己的搜索渠道',
      ...
      try {
        ...
        return {
          ok: false,
          errorCode: 'UPSTREAM_UNAVAILABLE',
          message: 'Zbot AI 搜索服务连接失败，请稍后再试',
          requestStarted: true,
        };
      }
      ...
      try {
        ...
        return {
          ok: false,
          errorCode: 'UPSTREAM_UNAVAILABLE',
          message: 'Zbot AI 搜索响应传输中断，请稍后再试',
          requestStarted: true,
          status: response.status,
          ...(requestId ? { requestId } : {}),
        };
