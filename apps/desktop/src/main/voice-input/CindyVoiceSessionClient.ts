...
export class CindyVoiceRunContext {
  ...
  async createRefinerTarget(
    ...
    if (this.refinerUnavailableOnServer) {
      throw new Error('Zbot voice service on this server does not support managed refinement yet.');
    }
    ...
    if (!token) throw new Error('Zbot login is required for voice refinement.');
    const baseUrl = getClientEndpoint('voiceApiBaseUrl');
    if (!baseUrl) throw new Error('Zbot voice service is unavailable in this region.');
    return {
...
async function createCindyVoiceSession(input: {
  ...
  requireAppCapability('canUseCindyAccountServices', 'Zbot voice requires a Zbot account.');
  if (!getClientEndpoint('voiceApiBaseUrl')) {
    throw new Error('Zbot voice service is unavailable in this region.');
  }
  const request = {
  ...
  if (
    ...
    throw new Error('Zbot voice service returned an invalid session.');
  }
  return session;
}
