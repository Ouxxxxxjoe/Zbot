...
> 源码目录、二进制与产物均已统一为 `zbot-updater`（经 owner 确认，
> docs/dev-rules/cindy-updater.md）。

Tauri-based Windows updater for `Zbot`. Replaces the inline `.cmd` script
...
- Self-update via copy-to-%TEMP% pattern: the updater is copied to
  `%TEMP%\zbot-updater-{ts}.exe` before launch, so the in-`resources/` copy is
...
- Official Windows packaging **always rebuilds it from this source**:
  `forge.config.ts` (`buildZbotUpdater`) runs `cargo build --release` during
  `prePackage`, patches the embedded manifest, copies the result to
  `apps/desktop/resources/zbot-updater.exe`, and hard-fails if the toolchain
  is missing — a release never ships a stale binary.
- **No prebuilt copy is committed.** `apps/desktop/resources/zbot-updater.exe`
...
- To produce one locally (Windows, needs the prerequisites at the bottom):
  ...
  ```bash
  cargo build --release --manifest-path apps/desktop/zbot-updater/src-tauri/Cargo.toml
  # output: apps/desktop/zbot-updater/src-tauri/target/release/zbot-updater.exe
  ```
...
zbot-updater.exe \
  ...
  --log       <userData>/logs/zbot-update.log \
  --lock      <userData>/updates/.updating \
  --theme     light|dark|auto      # default: auto
```

`--theme` mirrors the user's current Zbot theme preference into the
...
on a light OS who has selected dark mode in Zbot would briefly see a
...
- Path is whatever `--log` points at; convention is
  `<userData>/logs/zbot-update.log` so the file lives next to the main
...
```
cd apps/desktop/zbot-updater
pnpm install              # pulls @tauri-apps/cli
pnpm tauri build          # produces target/release/zbot-updater.exe
```
