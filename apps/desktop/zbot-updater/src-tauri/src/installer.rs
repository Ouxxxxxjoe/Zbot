...
fn run_inner<F: FnMut(InstallerEvent)>(
    ...
    let extract_dir = args.workdir.join(format!("zbot-update-extract-{ts}"));
    ...
    let backup_dir = args.workdir.join(format!("zbot-update-rollback-{ts}"));
...
pub fn sweep_stale_temp_dirs() {
    ...
    //   - zbot-update-{ts}/           current workdir layout (2026-07 rebrand)
    ...
    let prefixes = ["zbot-update", "xdt-update"];
...
fn workdir_ts(workdir: &Path) -> String {
    workdir
        .file_name()
        .and_then(|n| n.to_str())
        .and_then(|n| n.strip_prefix("zbot-update-").or_else(|| n.strip_prefix("xdt-update-")))
        .map(|s| s.to_string())
...
fn needs_elevation(app_dir: &Path) -> io::Result<bool> {
    ...
    let probe = app_dir.join(format!(".zbot-update-write-probe-{}", std::process::id()));
